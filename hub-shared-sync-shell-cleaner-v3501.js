...
(function(){
  ...
  const isEmptyEmergencyShell=record=>{
    ...
    const contactName=valueOf(record,'contactName');
  ...
  async function removeShells(){
    ...
    if(!confirm(`Remove ${shells.length} generic Emergency Contact shell${shells.length===1?'':'s'} from this phone? Any emergency contact with a contact name will be kept.`))return -1;
  ...
  function attach(){
    ...
    box.innerHTML='<strong>Receiver repair</strong><p style="margin:5px 0 10px">Use only when this phone shows generic Emergency Contact records with no contact names.</p><button type="button" data-remove-empty-emergency-shells style="width:100%;padding:11px;border:1px solid #b98b55;border-radius:9px;background:#fff;font-weight:800">Remove generic Emergency Contact shells</button><p data-shell-status style="margin:8px 0 0"></p>';
    ...
    box.querySelector('[data-remove-empty-emergency-shells]')?.addEventListener('click',async()=>{
      try{
        ...
        if(n>0)status.textContent=`Removed ${n} generic shell${n===1?'':'s'}. Now import a fresh Shared Records package from Glenn's master phone.`;
        else if(n===0)status.textContent='No generic Emergency Contact shells were found.';
        else status.textContent='No changes made.';
      }catch(error){status.textContent=error?.message||'Unable to remove the generic shells.';}
    });
    return true;
  }
