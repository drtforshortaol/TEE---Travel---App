...
(function(){
  ...
  async function removeShells(){
    ...
    const ids=new Set(shells.map(r=>r.recordId).filter(Boolean));
    data.records=data.records.filter(r=>!isEmptyEmergencyShell(r)||(!ids.size&&false));
  ...
  function attach(){
    const dialog=document.getElementById('hubSharedSyncDialogV3500');
    if(!dialog||dialog.querySelector('[data-remove-empty-emergency-shells]'))return;
    const receive=dialog.querySelector('section:nth-of-type(2)')||dialog.querySelector('[data-import]')?.closest('section');
    if(!receive)return;
  ...
  syncButton.addEventListener('click',()=>setTimeout(attach,80));
