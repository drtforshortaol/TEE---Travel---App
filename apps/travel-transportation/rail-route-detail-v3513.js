...
(() => {
  const DAYS = [
    ...
    },
    {
      ...
      legs:[{train:"TRN / IR70 2616", depart:"09:09", arrive:"09:51", from:"Luzern", to:"Zürich HB", stops:["Luzern","Zug","Zürich HB"]}], transfers:[], note:"Direct train; no transfer."
    },
    {
      id:"rail-oct6", date:"Oct 6", title:"Zurich HB → Zurich Airport",
      legs:[{train:"IC 513", depart:"10:02", arrive:"10:11", from:"Zürich HB", to:"Zürich Flughafen", stops:["Zürich HB","Zürich Flughafen"]}], transfers:[], note:"Direct airport train; Eurail shows seat reservations optional."
    }
  ];
  ...
  const glanceHtml=d=>{let out='';d.legs.forEach((leg,i)=>{if(i)out+=transferHtml(d.transfers[i-1]);out+=`<div class="tee-rail-line">${routeLine(leg.stops)}</div>`;});return `<div class="tee-rail-glance"><h4>At a Glance</h4>${out}</div>`;};
  ...
  function render(){injectStyles();const transport=document.getElementById('transportMount');if(!transport)return;document.getElementById('teeRailReference')?.remove();const section=document.createElement('section');section.id='teeRailReference';section.innerHTML=`<div class="tee-rail-reference-head"><span class="tee-rail-date">PART 2 RAIL</span><h2 style="margin:.2rem 0 .45rem">Rail Routes, Transfers & Maps</h2><p style="margin:0">Use the At-a-Glance line while riding. Open the full details before or during a transfer for the next train, layover time, and station map.</p></div>${DAYS.map(d=>`<article id="${d.id}" class="tee-rail-day"><span class="tee-rail-date">${esc(d.date)}</span><h3>${esc(d.title)}</h3>${glanceHtml(d)}${mapHtml(d)}<div class="tee-rail-details"><h4 style="margin:14px 0 6px">Full Details</h4>${detailHtml(d)}</div><p class="tee-rail-note">${esc(d.note)}</p><p class="tee-rail-live">Day-of rule: confirm the live departure board / Rail Planner before each transfer; platforms can change.</p><a class="tee-rail-back" href="../travel-daily-operations/index.html#teeRailTodayGlance">← Back to Today Quick Glance</a></article>`).join('')}`;transport.parentNode.insertBefore(section,transport);const hash=location.hash;if(hash&&/^#rail-/.test(hash)){requestAnimationFrame(()=>document.querySelector(hash)?.scrollIntoView({behavior:'smooth',block:'start'}));}}
