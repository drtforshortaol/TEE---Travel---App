...
(() => {
  ...
  let queued=false;function queueMount(){if(queued)return;queued=true;requestAnimationFrame(()=>{queued=false;mountTop();});}
  function start(){mountTop();const main=document.querySelector('main');if(main){const observer=new MutationObserver(queueMount);observer.observe(main,{childList:true,subtree:true,characterData:true});}document.getElementById('previewDateSelect')?.addEventListener('change',()=>setTimeout(queueMount,0));document.getElementById('useActualDateBtn')?.addEventListener('click',()=>setTimeout(queueMount,0));setTimeout(queueMount,250);setTimeout(queueMount,900);}
