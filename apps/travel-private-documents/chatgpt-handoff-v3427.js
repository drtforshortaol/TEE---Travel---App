...
(function(){
  ...
  function configurePanel(){
    ...
    if(help&&!help.dataset.teeChatgptOwner){
      help.dataset.teeChatgptOwner='v3427';help.open=true;
    ...
    const analyze=document.getElementById('teeV3415OpenChat');if(analyze)analyze.textContent=mobile?'Analyze with ChatGPT':'Open ChatGPT + Copy Request';
    const paste=document.getElementById('teeV3415PasteResults');if(paste)paste.textContent='Paste Results';
  ...
  const observer=new MutationObserver(()=>configurePanel());observer.observe(document.documentElement,{childList:true,subtree:true,attributes:true,attributeFilter:['hidden']});setTimeout(()=>observer.disconnect(),30000);
  configurePanel();
})();