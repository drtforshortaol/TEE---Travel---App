...
(function(){
  ...
  async function importFile(file){
    try{
      if(!file)return;
      if(!isUnlocked())throw new Error('Unlock the Secure Vault first.');
      const code=prompt('Enter the Shared Records sync code from Glenn’s master iPhone.');
      if(code===null)return;
  ...
  function ensureUi(){
    ...
    dialog.innerHTML=`<div style="padding:18px;background:#fff">
      ...
      <div style="margin:14px 0;padding:12px 14px;border:1px solid #d8c895;border-radius:12px;background:#fff9e8"><strong>Trip rule: Glenn’s iPhone is the master Shared source.</strong><p style="margin:5px 0 0;color:#5f6259">Normally make Shared changes on Glenn’s phone first. Vicki’s phone and Michael/Susan’s phones receive the Shared package.</p></div>
      <details open style="border:1px solid #d9e3e4;border-radius:14px;padding:12px 14px;margin:12px 0;background:#fbfcfc"><summary style="font-weight:800;cursor:pointer">How to sync in the field — step by step</summary><div style="margin-top:10px;line-height:1.45"><p style="margin:0 0 6px"><strong>On Glenn’s master iPhone — SEND</strong></p><ol style="margin:0 0 12px;padding-left:22px"><li>Open TEE Hub and unlock the Secure Vault.</li><li>Tap <strong>Sync Shared Records</strong>.</li><li>Tap <strong>Create / Share Shared Records</strong>.</li><li>Choose <strong>AirDrop</strong> and select the receiving traveler’s iPhone.</li><li>Keep the sync code shown here. Give the code separately — verbally or by text.</li></ol><p style="margin:0 0 6px"><strong>On the other traveler’s iPhone — RECEIVE</strong></p><ol style="margin:0 0 12px;padding-left:22px"><li>Accept the AirDrop. If iPhone asks where to keep the file, save it in <strong>Files</strong>.</li><li>Open TEE Hub and unlock the Secure Vault.</li><li>Tap <strong>Sync Shared Records</strong>.</li><li>Tap <strong>Choose Shared Records File</strong>.</li><li>Select the AirDropped TEE Shared Records file from <strong>Files</strong>.</li><li>Enter the sync code from Glenn’s phone.</li><li>Confirm TEE reports the sync completed.</li><li>Open Vault Records or Quick Reference and verify the needed Shared information is present.</li></ol><p style="margin:0"><strong>Important:</strong> Do not upload this Shared file to GitHub. AirDrop is preferred when the travelers are together.</p></div></details>
      <p data-count style="margin:14px 0;padding:10px 12px;border-radius:10px;background:#f5f8f8"></p>
      <section style="border:1px solid #d9e3e4;border-radius:14px;padding:14px;margin:12px 0"><h3 style="margin:0 0 6px">Glenn’s master phone — Send</h3><p style="margin:0 0 12px;color:#5d6e74">Create one encrypted Shared Records file, then AirDrop it to the other traveler’s phone. Keep the sync code separate.</p><button type="button" data-export style="width:100%;padding:13px;border:0;border-radius:11px;background:#1f5a67;color:#fff;font-weight:800;font-size:16px">Create / Share Shared Records</button><div data-code-row hidden style="margin-top:12px"><label style="display:block;font-weight:800">Sync code<input data-code readonly style="display:block;width:100%;box-sizing:border-box;margin-top:6px;padding:12px;font:700 18px ui-monospace,monospace;border:1px solid #b9c9ce;border-radius:10px"></label><div style="display:flex;gap:8px;margin-top:8px"><button type="button" data-copy-code style="flex:1;padding:10px;border:1px solid #adc1c7;border-radius:10px;background:#fff;font-weight:700">Copy code</button><button type="button" data-share-again style="flex:1;padding:10px;border:1px solid #adc1c7;border-radius:10px;background:#fff;font-weight:700">Share file again</button></div></div></section>
      <section style="border:1px solid #d9e3e4;border-radius:14px;padding:14px;margin:12px 0"><h3 style="margin:0 0 6px">Other traveler’s phone — Receive</h3><p style="margin:0 0 12px;color:#5d6e74">Choose the AirDropped Shared Records file from Files. TEE merges newer Shared records and leaves private records untouched.</p><button type="button" data-import style="width:100%;padding:13px;border:0;border-radius:11px;background:#2f6b4f;color:#fff;font-weight:800;font-size:16px">Choose Shared Records File</button><input data-import-file type="file" hidden></section>
  ...
  function open(){
    ...
    else setStatus('Ready. Glenn’s phone normally sends; the other traveler’s phone receives.','info');
  ...
  window.TEESharedSyncV3490=Object.freeze({version:'3.4.92',open,sharedRecords,mergeShared});
})();
